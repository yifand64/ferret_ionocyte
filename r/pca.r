#!/usr/bin/Rscript
# Principal components analysis utilities
# adam, april 9, 2015
library(ggplot2)
library(scales)
library(grid)
...
sig.pcs.perm <- function (dat, B = 100, threshold = 0.05,
                                        randomized=F,  
                                        verbose=TRUE, seed = NULL, fix.bug=F,
    ...
    #### [watch out for this line! this performs row-wise scaling, but because the input matrix is transposed, this is actually column-wise!! [Sep12, 2016]]
    if(!fix.bug){
...
#### find how many PCs are robust to perturbation of the data with random noise (adapted from Atray Dixit's code)
sig.pcs.noise <- function(DGEZ,noiselevels=seq(from=-2, to=2, length=20),reps=3,sig_pcs=40)
{
...
variance.explained.plot <- function(pca, show_n_pcs=6)
{
  	...
  	var_plot = ggplot(x, aes(pc_index, variance)) + 
  	   ...
  	   #             size = 6)  + 
  	    ...
  	    theme_cowplot() + xlab("PC") + ylab("% Variance explained") + 
...
get.loaded.genes <- function(loadings, components=1:6, n_genes=20)
{
